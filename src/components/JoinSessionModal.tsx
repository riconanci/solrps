...
export function JoinSessionModal({ open, onClose, session, onJoined }: JoinSessionModalProps) {
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      // Check if game was resolved immediately
      if (result.gameResolved && result.result) {
        // Show results modal instead of just closing
        setGameResult(result.result);
        setShowResults(true);
        onClose(); // Close join modal
      } else {
        // Old flow - just close and refresh
        onJoined?.(session.id);
        onClose();
      }
  ...
  const handleResultsClose = () => {
    ...
    setGameResult(null);
    onJoined?.(session.id); // Refresh lobby
  };
