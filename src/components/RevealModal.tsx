      ...
      <div className="w-full max-w-md rounded-2xl border border-white/15 bg-neutral-950 p-4">
        ...
        <div className="space-y-2 text-sm">
          <label className="block">
            ...
            <input className="mt-1 w-full rounded bg-white/10 p-2"
              value={salt} onChange={(e) => setSalt(e.target.value)} placeholder="auto-loaded if available" />
          </label>
          <label className="block">
            <span className="text-neutral-300">Moves (CSV)</span>
            <input className="mt-1 w-full rounded bg-white/10 p-2"
              value={movesCsv} onChange={(e) => setMovesCsv(e.target.value)} placeholder="R,P,S (for 3 rounds)" />
          </label>
          {err && <div className="rounded border border-red-400/30 bg-red-500/10 p-2 text-red-300">{err}</div>}
        </div>
        <div className="mt-4 flex justify-end gap-2">
          <button className="rounded-lg bg-white/10 px-3 py-1 hover:bg-white/20" onClick={onClose} disabled={busy}>Cancel</button>
          <button className="rounded-lg bg-white/20 px-3 py-1 hover:bg-white/30" onClick={doReveal} disabled={busy}>
